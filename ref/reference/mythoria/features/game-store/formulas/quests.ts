import type { Player, Quest, QuestBoardSlot, Rarity } from "@/features/types";
import { rollDice, rngFloat } from "@/features/game-store/rng";
import { selectEffectiveStats } from "@/features/game-store/selectors";
...
> = {
  ...
  gather: { primary: "arcane", secondary: null, item: "accessory" },
  stealth: { primary: "speed", secondary: "luck", item: "armor" },
  fortune: { primary: "luck", secondary: "crit", item: "avatar" },
  guardian: { primary: "defense", secondary: null, item: "mount" },
};
...
const ITEM_ONLY_STATS = new Set(["luck", "speed"]);
...
export function canStartQuest(
  ...
  if ((player.aether || 0) < cost) return { ok: false, reason: "Not enough $AETHER", cost };
  return { ok: true, cost };
}
...
// For gather/artifact quests, relic rolls are mirrored as shards (same rarity, same amounts).
const SHARD_QUEST_TYPES = new Set(["gather"]);

export function collectQuestRewards(
  quest: Quest,
  player: Player,
  seed: string
): { relics: Partial<Record<Rarity, number>>; shards?: Partial<Record<Rarity, number>>; xp: number; log: string[] } {
  ...
  // For gather/artifact quests, mirror relics as shards of the same rarity/amount.
  const shards: Partial<Record<Rarity, number>> | undefined = SHARD_QUEST_TYPES.has(quest.quest_type)
    ? { ...relics }
    : undefined;

  return { relics, shards, xp: TIER_XP[quest.tier as keyof typeof TIER_XP], log };
}
