 ...
 *
 * Pure event — fight a planet boss for crate/consumable/relic drops.
 * Operates on plain state snapshots; no DB calls.
 */

import { now } from "@/lib/clock";
import type { BossDatum, Player, Rarity } from "@/features/types";
...
const PLANETS: Record<string, { essence: number; consumableThreshold: number; rarityThresholds: number[] }> = {
...
export function canFightBoss(player: Player, planet: string): { ok: boolean; reason?: string } {
  ...
  if ((player.essence || 0) < cfg.essence) return { ok: false, reason: "Not enough essence" };
  const nowMs = now();
...
export function fightBoss(
  ...
  if (!hit) {
    ...
    let amount: number;
    if (roll2 <= 70) {
      rarity = "common"; amount = Math.max(rngFloat(seed + "-amt") * 1.25 * luckMod + 1, 0.1);
    } else if (roll2 <= 90) {
      rarity = "uncommon"; amount = Math.max(rngFloat(seed + "-amt") * 1.0 * luckMod + 1, 0.1);
    } else if (roll2 <= 98) {
      rarity = "rare"; amount = Math.max(rngFloat(seed + "-amt") * 0.75 * luckMod + 1, 0.1);
    } else if (roll2 <= 99) {
      rarity = "epic"; amount = Math.max(rngFloat(seed + "-amt") * 0.5 * luckMod + 1, 0.1);
    } else {
      rarity = "legendary"; amount = Math.max(0.1 * luckMod, 0.1);
    }
    ...
    const updatedRelics = existing
      ? relics.map((r) => r.type.toLowerCase() === rarity ? { ...r, amount: r.amount + amount } : r)
      : [
          ...relics,
          {
    ...
    useGameStore.setState((s) => ({
      players: {
        ...
        [user]: {
          ...player,
          essence: (player.essence || 0) - cfg.essence,
          boss_data: bossData,
  ...
  if (dropRoll <= cfg.consumableThreshold) {
    ...
    useGameStore.setState((s) => ({
      ...
      inventory: { ...s.inventory, [user]: { ...s.inventory[user], consumables: updatedConsumables } },
    }));
  ...
  useGameStore.setState((s) => ({
    ...
    inventory: {
      ...
      [user]: {
        ...s.inventory[user],
        crates: [...crates, { _id: `${user}-crate-${nowMs}`, rarity, owner: user, acquired: nowMs }],
      },
    },
  }));
