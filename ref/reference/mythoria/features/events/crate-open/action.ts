/**
 * features/events/crate-open/action.ts
 *
 * Pure event — open a crate and mint a randomised item into the player's inventory.
 * Operates on plain state snapshots; no DB calls.
 */

import { now } from "@/lib/clock";
import type { Crate, Item, ItemType } from "@/features/types";
import { useGameStore } from "@/features/game-store/game-store";
import { rollItemAttributes, rollItemRarity } from "@/features/game-store/formulas/crates";
import { createSeed, rngInt } from "@/features/game-store/rng";

const ITEM_TYPES: ItemType[] = ["weapon", "armor", "mount", "accessory", "avatar"];
...
export function openCrate(
  ...
  const item: Item = {
    ...
    print: 1,
    max_supply: 10000,
    description: `A ${rarity} ${type} forged from the ruins of Mythoria.`,
  ...
  useGameStore.setState((s) => ({
    inventory: {
      ...s.inventory,
      [user]: { ...inventory, crates: nextCrates, items: [...inventory.items, item] },
    },
    nftLogs: {
      ...s.nftLogs,
      [user]: [
