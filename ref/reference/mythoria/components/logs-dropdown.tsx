...
/** Resolve a TanStack-style route with params into a plain href.
 *  e.g. { to: "/$user/forge_logs", params: { user: "alice" } } → "/alice/forge_logs"
 */
function resolveHref(to: string, params?: Record<string, string>): string {
  if (!params) return to;
  return Object.entries(params).reduce(
    (href, [key, val]) => href.replace(`$${key}`, val),
    to,
  );
}
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div
          role="menu"
          className="absolute right-0 top-full mt-1 min-w-[180px] bg-background border border-border rounded-md shadow-lg"
            style={{ zIndex: 200 }}
        >
          {items.map((it) => (
            <Link
              key={it.to + JSON.stringify(it.params ?? {})}
              href={resolveHref(it.to, it.params)}
