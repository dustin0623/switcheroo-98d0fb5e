...
describe("FlushScheduler", () => {
  ...
  it("uses the real loaded map version as the first CAS baseline", async () => {
    mocks.persistMineAction.mockResolvedValue({
      status: "ok",
      stageAdvanced: false,
      committedState: state(8, 20),
    });
    const session = entry();
    session.state = state(8, 20);
    session.lastFlushedState = state(7, 10);
    const scheduler = new FlushScheduler(new SessionStore());

    await scheduler.flushOne(session);

    expect(mocks.persistMineAction).toHaveBeenCalledWith(
      expect.objectContaining({ prevState: expect.objectContaining({ mapVersion: 7 }) }),
    );
    expect(session.lastFlushedState?.mapVersion).toBe(8);
  });

  it("does not fabricate a version-zero baseline for a new unpersisted map", async () => {
    const session = entry();
    session.lastFlushedState = null;
    session.isNew = true;
    const scheduler = new FlushScheduler(new SessionStore());

    await scheduler.flushOne(session);

    expect(mocks.persistMineAction).not.toHaveBeenCalled();
    expect(session.dirty).toBe(true);
  });
