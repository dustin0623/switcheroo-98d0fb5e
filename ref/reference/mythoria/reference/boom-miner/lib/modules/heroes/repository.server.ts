...
import { MINT_COST, MAX_ON_MAP, RARITY_RECOVERY_FRACTION, RECOVERY_INTERVAL_SECONDS, canDeployEnergy } from "@/lib/constants/game";
...
/**
 * Immediately persists `onMap: false` and the hero's current in-memory energy
 * in a single targeted write. Called synchronously before the undeploy ACK is
 * sent so that energy is durable even if the player closes the tab immediately
 * after undeploying (Phase 3).
 *
 * Unlike setHeroOnMap this function does NOT re-check energy thresholds or
 * MAX_ON_MAP limits — those validations happen in the WS handler before this
 * write fires. It only needs to atomically record the authoritative depleted
 * energy value alongside the onMap flag change.
 *
 * Returns true when a document was matched and updated, false when the hero no
 * longer exists or does not belong to this wallet (caller should reject the
 * undeploy).
 */
export async function setHeroOffMap(
  wallet: string,
  heroId: string,
  currentEnergy: number,
): Promise<boolean> {
  await connectDatabase();
  // Write lastRegenAt alongside the energy so catchUpOfflineRegen has an
  // accurate baseline if the player disconnects and reconnects later.
  // Without this, legacy heroes with lastRegenAt=0 would fall back to
  // updatedAt — which is correct but less precise than "now".
  const result = await HeroModel.updateOne(
    { _id: heroId, ownerWallet: wallet },
    { $set: { onMap: false, currentEnergy, lastRegenAt: Date.now() } },
  );
  return result.matchedCount === 1;
}

/**
 * Persists current energy for heroes that are off-map (resting heroes whose
 * energy is tracked in the session's heroEnergy map but not in state.heroes).
 *
 * Uses `$max` instead of `$set` so that regen ticks written directly by
 * RegenScheduler are never overwritten by a stale in-memory value: if the DB
 * already has a higher energy (from regen that ran after the last undeploy
 * write), the update is a no-op for that field. The `onMap: false` flag is
 * always written to guard against any flag inconsistency.
 *
 * Called by FlushScheduler.performFlush after persistMineAction succeeds, as
 * the second leg of the consolidated flush (Phase 5).
 */
export async function persistOffMapHeroEnergies(
  wallet: string,
  entries: Array<{ heroId: string; energy: number }>,
): Promise<void> {
  if (entries.length === 0) return;
  await connectDatabase();
  await Promise.all(
    entries.map(({ heroId, energy }) =>
      HeroModel.updateOne(
        { _id: heroId, ownerWallet: wallet },
        {
          $max: { currentEnergy: energy },
          $set: { onMap: false },
        },
      ),
    ),
...
export async function setHeroOnMap(
  ...
  if (onMap) {
    // Cannot deploy until the hero has recovered to at least 50% energy.
    if (!canDeployEnergy(hero.currentEnergy, hero.maxEnergy)) return null;
...
export async function catchUpOfflineRegen(wallet: string): Promise<void> {
  ...
  const ops = heroes
    .filter((h) => h.currentEnergy < h.maxEnergy)
    .map((h) => {
      // Determine the best available baseline timestamp:
      //   1. lastRegenAt — set by every regenHeroEnergy / catchUpOfflineRegen write.
      //      Most accurate: reflects exactly when energy was last persisted.
      //   2. updatedAt   — Mongoose timestamps, set on every document write.
      //      Good proxy when the hero was recently undeployed (Phase 3 writes both
      //      currentEnergy and lastRegenAt=0 for legacy docs created before the
      //      lastRegenAt field existed).
      //   3. createdAt   — absolute worst case for a brand-new hero that has never
      //      been written since creation. Better than skipping entirely.
      //
      // NOTE: we do NOT skip when lastTick is 0 / absent. If no timestamp is
      // available we fall back to createdAt. The energy gain is capped at
      // maxEnergy, so the worst that can happen is a hero arriving at full energy —
      // which is correct behaviour (they've been resting since creation).
      const updatedAtMs = h.updatedAt ? new Date(h.updatedAt).getTime() : 0;
      const createdAtMs = h.createdAt ? new Date(h.createdAt).getTime() : 0;

      let lastTick = 0;
      if (h.lastRegenAt > 0) {
        lastTick = h.lastRegenAt;
      } else if (updatedAtMs > 0) {
        // updatedAt is the last time any field changed — this includes the
        // setHeroOffMap write that persisted the depleted energy, so it is a
        // conservative (slightly too late) baseline that slightly under-grants
        // regen rather than over-granting. That is the safe direction.
        lastTick = updatedAtMs;
      } else if (createdAtMs > 0) {
        lastTick = createdAtMs;
      } else {
        // Truly no timestamp at all — cannot compute regen safely; skip.
        return null;
      }
