...
describe("heroDeploy", () => {
  ...
  it("rejects a hero recovered to just below 50% energy", () => {
    const hero = makeHero({ currentEnergy: 249, maxEnergy: 500 });
    const result = heroDeploy({ state: makeState([hero]), action: { heroId: "hero-1" } });
    expect(result.ok).toBe(false);
    expect(result.code).toBe("INSUFFICIENT_ENERGY");
  });

  it("deploys a hero at exactly 50% energy", () => {
    const hero = makeHero({ currentEnergy: 250, maxEnergy: 500 });
    const result = heroDeploy({ state: makeState([hero]), action: { heroId: "hero-1" } });
    expect(result.ok).toBe(true);
    expect(result.hero?.onMap).toBe(true);
  });
