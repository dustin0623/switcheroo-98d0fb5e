 ...
 *  3. Hero must have recovered to at least 50% of its max energy.
 *  4. Deployed count must be below MAX_ON_MAP.
 */

import { MAX_ON_MAP, minDeployEnergyFor, canDeployEnergy, MIN_DEPLOY_ENERGY_FRACTION } from "@/lib/constants/game";
...
export function heroDeploy({
  ...
  if (!canDeployEnergy(hero.currentEnergy, hero.maxEnergy)) {
    const pct = Math.round(MIN_DEPLOY_ENERGY_FRACTION * 100);
    return {
      ok:    false,
      error: `Hero must recover to ${pct}% energy (needs ${minDeployEnergyFor(hero.maxEnergy)}) before deploying`,
