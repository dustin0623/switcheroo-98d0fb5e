...
export interface MineMutationDelta {
  changedNodes: Record<string, MapNodeSnapshot>;
  changedHero: { heroId: string; currentEnergy: number } | null;
  coinDelta: number;
  coinsTotal: number;
...
export function deriveMineMutationDelta(prevState: MineState, nextState: MineState): MineMutationDelta {
  const changedNodes = Object.fromEntries(
    Object.entries(nextState.nodes).filter(([key, node]) =>
      JSON.stringify(prevState.nodes[key]) !== JSON.stringify(node),
    ),
  );
  const changedHeroes = Object.entries(nextState.heroes).filter(([heroId, hero]) =>
    prevState.heroes[heroId]?.currentEnergy !== hero.currentEnergy,
  );
  if (changedHeroes.length > 1) throw new Error("A detonation may mutate only one hero");
  const changedHero = changedHeroes[0]
    ? { heroId: changedHeroes[0][0], currentEnergy: changedHeroes[0][1].currentEnergy }
    : null;
  return {
    changedNodes,
    changedHero,
    coinDelta: nextState.coins - prevState.coins,
    coinsTotal: nextState.coins,
    clearedChests: Object.values(nextState.nodes).filter((node) => node.kind === "chest" && node.destroyed).length,
    destroyedNodes: nextState.destroyedNodes,
...
async function persistInTransaction(
  ...
  const delta = deltaOnly && !stageComplete ? deriveMineMutationDelta(prevState, nextState) : null;
  const deltaNodeUpdates = delta
    ? Object.fromEntries(Object.entries(toStoredNodes(delta.changedNodes)).map(([key, node]) => [`nodes.${key}`, node]))
    : null;

  const mapResult = await StageMapModel.updateOne(
    mapVersionFilter(wallet, prevState),
    {
      $set: stageTransition?.mapUpdate ?? (delta ? {
        ...deltaNodeUpdates,
        clearedChests: delta.clearedChests,
        destroyedNodes: delta.destroyedNodes,
        mapVersion: delta.mapVersion,
        lastActionAt: delta.lastActionAt,
      } : {
  ...
  const heroesToPersist: Array<[string, { currentEnergy: number }]> = delta
    ? (delta.changedHero ? [[delta.changedHero.heroId, { currentEnergy: delta.changedHero.currentEnergy }]] : [])
    : Object.entries(nextState.heroes);
  for (const [heroId, hero] of heroesToPersist) {
  ...
  const playerResult = await PlayerModel.updateOne(
    { wallet },
    delta
      ? { $inc: { coins: delta.coinDelta }, $set: { stage: committedState.stage } }
...
export async function persistJournaledMineAction(opts: PersistOptions): Promise<PersistResult> {
  await connectDatabase();
  const session = await mongoose.startSession();
  try {
    let committed: { stageAdvanced: boolean; committedState: MineState } | undefined;
    await session.withTransaction(async () => {
      committed = await persistInTransaction(opts, session, true);
    });
    if (!committed) {
    ...
    const message = isTransactionUnsupportedError(error)
      ? "Journal compaction requires MongoDB transaction support"
      : String(error);
    return { status: "failed", errors: [message], stageAdvanced: false };
  } finally {
    await session.endSession();
  }
}
