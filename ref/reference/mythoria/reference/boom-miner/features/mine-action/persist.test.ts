...
import { deriveMineMutationDelta, persistJournaledMineAction, persistMineAction } from "./persist";
...
describe("persistMineAction", () => {
  ...
  it("derives and persists only server-authoritative changed paths", async () => {
    const prevState = state(0, 0);
    const nextState = state(1, 10);
    expect(deriveMineMutationDelta(prevState, nextState)).toMatchObject({
      coinDelta: 10,
      changedHero: { heroId: "hero1", currentEnergy: 4 },
      mapVersion: 1,
    });

    const result = await persistJournaledMineAction({ prevState, nextState, stageComplete: false });
    expect(result.status).toBe("ok");
    expect(mocks.mapUpdate.mock.calls[0][1].$set).toHaveProperty("nodes.1,1");
    expect(mocks.mapUpdate.mock.calls[0][1].$set).not.toHaveProperty("nodes");
    expect(mocks.playerUpdate.mock.calls[0][1]).toEqual({
      $inc: { coins: 10 },
      $set: { stage: 1 },
    });
  });

  it("fails closed when journal compaction lacks transaction support", async () => {
    mocks.withTransaction.mockRejectedValue(Object.assign(
      new Error("Transactions are not supported"),
      { code: 20 },
    ));
    const result = await persistJournaledMineAction({
      prevState: state(0, 0),
      nextState: state(1, 10),
      stageComplete: false,
    });
    expect(result).toMatchObject({ status: "failed", stageAdvanced: false });
    expect(mocks.mapUpdate).not.toHaveBeenCalled();
  });
