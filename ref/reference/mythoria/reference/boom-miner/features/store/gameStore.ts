...
export { MINT_COST, MAX_ON_MAP, ENERGY_PER_STAMINA, RECOVERY_FRACTION_PER_INTERVAL, RECOVERY_INTERVAL_SECONDS, MIN_DEPLOY_ENERGY_FRACTION, maxEnergyFor, minDeployEnergyFor, canDeployEnergy };
...
interface GameState {
  ...
  /**
   * True while locally-simulated demo heroes are running on the map. Demo
   * heroes are spawned when the player has zero real heroes; they never touch
   * coins, energy, the roster, or the server. Set by DemoManager.start()/stop().
   */
  demoMode: boolean;
  ...
  setHeroOnMap: (id, onMap) =>
    set((s) => {
      ...
      if (onMap) {
        ...
        // A hero must have recovered to at least 50% energy to be deployed.
        // This applies to recalled heroes (energy drained below 50%) and slept
        // heroes (0 energy) — both must regen back to 50% before redeploying.
        if (!canDeployEnergy(target.currentEnergy, target.maxEnergy)) return s;
      }
      return {
