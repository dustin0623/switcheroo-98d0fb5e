...
import { DemoManager, isDemoId } from "../demo/DemoManager";
...
export class TreasureScene extends Phaser.Scene {
  ...
  private demo: DemoManager | null = null;
  ...
  create(): void {
    ...
    if (!process.env.NEXT_PUBLIC_WS_URL) {
      ...
      this.wsSyncManager = new WSSyncManager((canonical) => {
        this.map.applyServerState(canonical);
        // Decide demo mode here — AFTER the first SESSION_STATE has
        // populated the roster — so a returning player who already owns
        // heroes never sees the demo at all.
        if (!this.demo && useGameStore.getState().roster.length === 0) {
    ...
    this.syncStore();
    // Demo mode is started inside the applyServerState callback above, only
    // after SESSION_STATE has hydrated the roster. This prevents a returning
    // player (who already owns heroes) from ever seeing the demo flash.
  }
  ...
  update(time: number, deltaMs: number): void {
    ...
    this.bombs.update(time, (b) => {
      // Demo bombs are visual-only: run the explosion animation and clear the
      // grid locally, but never touch energy, coins, or the server. This is
      // what keeps a demo play session entirely unreal until the player owns a
      // hero.
      if (isDemoId(b.ownerId)) {
        const { destroyed: demoDestroyed } = this.explosions.detonate(b.tileX, b.tileY, b.range, b.damage, b.ownerId);
        // Credit coins locally so the HUD counter ticks up during demo play,
        // giving a feel for what the player would earn. No WS event is emitted
        // and no energy is consumed — the demo remains purely cosmetic.
        let demoCoinGain = 0;
        for (const d of demoDestroyed) demoCoinGain += d.coins;
        if (demoCoinGain > 0) useGameStore.getState().addOptimisticCoins(demoCoinGain);
        return;
      }
    ...
    const decisionBucket = Math.floor(time / 100) % 4;
    for (const hero of this.heroes.heroes) {
      // Demo heroes are driven by DemoManager (DemoAI), not the real AI.
      if (isDemoId(hero.id)) continue;
      let heroBucket = 0;
      for (let i = 0; i < hero.id.length; i++) heroBucket = (heroBucket * 31 + hero.id.charCodeAt(i)) >>> 0;
      this.ai.update(hero, time, deltaSec, stepAlong, heroBucket % 4 === decisionBucket);
    }

    // Drive the demo swarm, and tear it down the instant the player acquires
    // their first real hero (e.g. after a mint pushes SESSION_STATE).
    if (this.demo?.isActive) {
      if (useGameStore.getState().roster.length > 0) {
        // Restart the scene so create() runs fresh with the real server map.
        // The socket is a module-level singleton and survives the restart;
        // SESSION_STATE fires again on the new manager and hydrates the real map.
        this.scene.restart();
      } else {
        this.demo.update(time, deltaSec);
      }
    }
    ...
    // Demo mode also advances stages — it just skips the WS flush (takes the Promise.resolve() branch below).
  ...
  private reconcileHeroes(): void {
    ...
    // Remove heroes no longer wanted (e.g. sent home to rest). Never touch
    // demo heroes — they are owned by DemoManager, not the roster.
    for (const h of [...this.heroes.heroes]) {
      if (isDemoId(h.id)) continue;
  ...
  private syncStore(): void {
    ...
    // Demo heroes are not real roster heroes — keep them out of the store
    // snapshot so nothing in the HUD counts or renders them.
    const snap = this.heroes.heroes.filter((h) => !isDemoId(h.id)).map((h) => ({
      id: h.id,
      tileX: h.tileX,
      tileY: h.tileY,
      state: h.state,
