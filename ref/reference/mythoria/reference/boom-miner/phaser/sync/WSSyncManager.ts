...
let sharedSeq = 0;
let sharedDetonationSeq = 0;

type PendingDetonation = {
  payload: BombDetonatePayload;
  status: "queued" | "sent";
  attempts: number;
  lastSentAt: number;
};

const MAX_PENDING_DETONATIONS = 100;
const pendingDetonations = new Map<number, PendingDetonation>();
...
export class WSSyncManager {
  ...
  private bindSocketListeners(socket: Socket): void {
    ...
    socket.on("connect_error", (err: Error) => {
      ...
      if (
        ...
        err.message === "AUTH_INVALID_SESSION"
      ) {
    ...
    socket.on(WS_EVENTS.SESSION_STATE, (payload: { canonicalState: SyncLogResponse["canonicalState"] }) => {
      ...
      mgr.resendPendingDetonations(socket);
    });
    ...
    socket.on(WS_EVENTS.BOMB_DETONATE_ACK, (payload: BombDetonateAckPayload) => {
      if (payload.sessionId !== getGameplaySessionId()) return;
      pendingDetonations.delete(payload.seq);
  ...
  private resendPendingDetonations(socket: Socket): void {
    const now = Date.now();
    for (const pending of [...pendingDetonations.values()].sort((a, b) => a.payload.seq - b.payload.seq)) {
      const retryDelay = Math.min(8_000, 500 * 2 ** Math.min(pending.attempts, 4));
      if (pending.status === "sent" && now - pending.lastSentAt < retryDelay) continue;
      pending.status = "sent";
      pending.attempts += 1;
      pending.lastSentAt = now;
      socket.emit(WS_EVENTS.BOMB_DETONATE, pending.payload);
      setTimeout(() => {
        if (socket.connected && pendingDetonations.has(pending.payload.seq)) {
          this.resendPendingDetonations(socket);
        }
      }, retryDelay);
    }
  }
  ...
  /**
   * Queues a whole-bomb detonation before transport. Unacknowledged envelopes
   * retain their stable sessionId/seq and are replayed after session sync.
   */
  sendBombDetonate(heroId: string, nodeKeys: string[], power: number): void {
    if (pendingDetonations.size >= MAX_PENDING_DETONATIONS) {
      console.error("[WSSyncManager] pending detonation queue is full");
      return;
    }
    ...
    pendingDetonations.set(payload.seq, pending);
