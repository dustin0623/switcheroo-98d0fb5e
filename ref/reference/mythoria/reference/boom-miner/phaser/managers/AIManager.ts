...
export class AIManager {
  protected map: MapManager;
  protected pathfinding: Pathfinding;
  protected bombs: BombManager;
  protected loot: LootManager;
  protected explosions: ExplosionManager;
  ...
  protected targetedBreakables = new Map<string, string>();
  private reservationExpiry = new Map<string, number>();
  private dangerRevision = "";
  private dangerTiles = new Set<string>();
  private pathGeneration = new WeakMap<Hero, number>();
  private targetFieldRevision = "";
  private targetField = new Map<string, { standAt: PathNode; breakable: PathNode }>();
  private dangerCacheHits = 0;
  private dangerCacheMisses = 0;
  private targetFieldHits = 0;
  private targetFieldMisses = 0;
  ...
  update(
    ...
    now: number,
    deltaSec: number,
    stepAlong: (h: Hero, dt: number) => boolean,
    allowDecision = true,
  ): void {
    this.expireReservations(now);
    // Movement interpolation remains per-frame; expensive decisions are bucketed.
    if (this.pending.get(hero)?.requested) return;

    switch (hero.state) {
      case HeroState.Searching:
        if (allowDecision) this.doSearch(hero, now);
        break;
      ...
      case HeroState.PlantBomb:
        if (allowDecision) this.doPlantBomb(hero, now);
        break;
  ...
  private doSearch(hero: Hero, now: number): void {
    // Release any prior reservation before picking a new target.
    this.targetedBreakables.delete(hero.id);
    this.reservationExpiry.delete(hero.id);
    const danger = this.getAllDangerTiles();
    const reserved = new Set(this.targetedBreakables.values());
    const fieldTarget = this.getTargetField(danger).get(`${hero.tileX},${hero.tileY}`);
    const found = fieldTarget && !reserved.has(`${fieldTarget.breakable.x},${fieldTarget.breakable.y}`)
      ? fieldTarget
      : this.findNearestReachableBreakable(hero.tileX, hero.tileY, danger, reserved);
    if (!found) {
    ...
    this.reservationExpiry.set(hero.id, now + 5_000);
  ...
  private getTargetField(danger: Set<string>): Map<string, { standAt: PathNode; breakable: PathNode }> {
    const revision = `${this.pathfinding.getRevision()}:${this.bombs.getRevision()}`;
    if (revision === this.targetFieldRevision) {
      this.targetFieldHits++;
      return this.targetField;
    }
    this.targetFieldMisses++;

    const field = new Map<string, { standAt: PathNode; breakable: PathNode }>();
    const queue: Array<{ position: PathNode; standAt: PathNode; breakable: PathNode }> = [];
    const dirs = [[1, 0], [-1, 0], [0, 1], [0, -1]];
    for (let y = 1; y < MAP_HEIGHT - 1; y++) {
      for (let x = 1; x < MAP_WIDTH - 1; x++) {
        if (!isDestructible(this.map.grid[y][x])) continue;
        for (const [dx, dy] of dirs) {
          const standAt = { x: x + dx, y: y + dy };
          const key = `${standAt.x},${standAt.y}`;
          if (!this.pathfinding.isWalkable(standAt.x, standAt.y) || danger.has(key) || field.has(key)) continue;
          const target = { standAt, breakable: { x, y } };
          field.set(key, target);
          queue.push({ position: standAt, ...target });
        }
      }
    }
    let cursor = 0;
    while (cursor < queue.length) {
      ...
      for (const [dx, dy] of dirs) {
        ...
        if (field.has(key) || danger.has(key) || !this.pathfinding.isWalkable(position.x, position.y)) continue;
        const target = { standAt: current.standAt, breakable: current.breakable };
        field.set(key, target);
        queue.push({ position, ...target });
      }
    }
    this.targetFieldRevision = revision;
  ...
  protected doPlantBomb(hero: Hero, _now: number): void {
  ...
  /** One immutable-by-convention danger set shared until bomb or map state changes. */
  protected getAllDangerTiles(): Set<string> {
    const revision = `${this.bombs.getRevision()}:${this.pathfinding.getRevision()}`;
    if (revision === this.dangerRevision) {
      this.dangerCacheHits++;
      return this.dangerTiles;
    }
    this.dangerCacheMisses++;
    const next = new Set<string>();
    for (const b of this.bombs.getBombs()) {
      for (const t of this.tilesInBlast(b.tileX, b.tileY, b.range)) next.add(t);
    }
    this.dangerRevision = revision;
    this.dangerTiles = next;
    return next;
  }

  private expireReservations(now: number): void {
    for (const [heroId, expiresAt] of this.reservationExpiry) {
      if (expiresAt > now) continue;
      this.reservationExpiry.delete(heroId);
      this.targetedBreakables.delete(heroId);
    }
  }


  protected hasBreakableInBlast(bx: number, by: number, range: number): boolean {
    const dirs = [[1,0],[-1,0],[0,1],[0,-1]];
    for (const [dx, dy] of dirs) {
  ...
  getMetrics(): Readonly<{
    dangerCacheHits: number;
    dangerCacheMisses: number;
    targetFieldHits: number;
    targetFieldMisses: number;
  }> {
    return {
      dangerCacheHits: this.dangerCacheHits,
      dangerCacheMisses: this.dangerCacheMisses,
      targetFieldHits: this.targetFieldHits,
      targetFieldMisses: this.targetFieldMisses,
    };
  }
  ...
  protected requestPath(
    ...
    const generation = (this.pathGeneration.get(hero) ?? 0) + 1;
    this.pathGeneration.set(hero, generation);
    const mapRevision = this.pathfinding.getRevision();
    const bombRevision = this.bombs.getRevision();
    ...
    void this.pathfinding.findPath(hero.tileX, hero.tileY, tx, ty).then((path) => {
      if (
        this.pathGeneration.get(hero) !== generation ||
        this.pathfinding.getRevision() !== mapRevision ||
        this.bombs.getRevision() !== bombRevision
      ) {
