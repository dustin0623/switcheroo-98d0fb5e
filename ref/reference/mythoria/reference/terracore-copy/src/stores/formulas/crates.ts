import type { Attributes, Item, ItemType, Rarity } from "@/mock/types";
import { generateRandomNumber, rngFloat, rngInt } from "@/stores/rng";
...
const ALL_ATTRS: (keyof Attributes)[] = ["damage", "defense", "engineering", "dodge", "crit", "luck"];
...
export function rollItemAttributes(type: ItemType, rarity: Rarity, seed: string): Attributes {
  ...
  const attrs: Attributes = { damage: 0, defense: 0, engineering: 0, dodge: 0, crit: 0, luck: 0 };
...
export function salvageValue(attributes: Attributes): number {
  return (
    attributes.damage / 2 +
    attributes.defense / 2 +
    attributes.engineering * 5 +
    attributes.dodge * 5 +
    attributes.crit * 5 +
    attributes.luck * 10
  );
...
export function forgeCost(item: Item): number {
  return salvageValue(item.attributes) * 0.0498 * item.level;
}

export function forgeAttributes(attrs: Attributes): Attributes {
  return {
    damage: attrs.damage * 1.05,
    defense: attrs.defense * 1.05,
    engineering: attrs.engineering * 1.05,
    dodge: attrs.dodge * 1.05,
