...
import { canAttack, checkDodge, computeCurrentAttacks, rollAttack } from "@/stores/formulas/combat";
import { computeCurrentScrap } from "@/stores/formulas/mining";
...
export function attackPlayer(
  ...
  const stashSize = (attacker.hiveEngineStake || 0) + 1;
  ...
  const updatedAttacker: Player = {
    ...attacker,
    scrap: attackerScrap + scrapToSteal,
    attacks: attacks.current - 1,
    lastregen: attacks.newLastregen,
    lastBattle: nowMs,
    cooldown: nowMs,
  ...
  const updatedTarget: Player = {
    ...target,
    scrap: Math.max(0, targetScrap - scrapToSteal),
    lastBattle: nowMs,
    cooldown: nowMs,
  };
