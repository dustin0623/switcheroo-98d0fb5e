...
import { Home, Info, ChevronRight, ChevronLeft, Clock, Lock, Eye, Coins, Shield, Swords, Wrench, Rocket, User } from "lucide-react";
import questsBg from "@/assets/quests-bg.jpg";
...
import { startQuest, collectQuest } from "@/stores/actions/quests";
import {
  canStartQuest,
...
export const Route = createFileRoute("/quests/")({
  head: () => ({
    meta: [
      { title: "Quests — Mythoria" },
      { name: "description", content: "Endless quest mode and boss fights on distant worlds." },
      { property: "og:title", content: "Quests — Mythoria" },
      { property: "og:description", content: "Endless quest mode and boss fights." },
    ],
  }),
  ...
  return (
    <PageShell>
      <section className="relative min-h-screen w-full overflow-hidden">
        <img src={questsBg} alt="" className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-4 py-3 z-10">
          <div className="flex items-center gap-2">
            ...
            <div className="h-9 px-3 rounded-md border border-border/60 bg-background/60 backdrop-blur flex items-center gap-2 text-xs font-semibold tracking-widest">
              ...
              <span className="text-muted-foreground">·</span>
              <span className="text-primary">{player ? Math.floor(player.hiveEngineScrap || 0).toLocaleString() : "—"}</span>
              <span className="text-muted-foreground">$MGOLD</span>
            </div>
          </div>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center min-h-screen pt-24 pb-12">
          <div className="text-center">
            <h1 className="text-3xl md:text-4xl font-bold tracking-[0.3em] text-foreground">
              DAILY MISSIONS
            </h1>
          ...
          <div className="relative w-full mt-10">
            ...
            <button
              ...
              className="hidden md:flex absolute left-4 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full bg-background/70 border border-border/60 backdrop-blur items-center justify-center text-foreground hover:border-primary hover:text-primary transition-colors"
            >
            ...
            <button
              ...
              className="hidden md:flex absolute right-4 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full bg-background/70 border border-border/60 backdrop-blur items-center justify-center text-foreground hover:border-primary hover:text-primary transition-colors"
            >
...
function MissionCard({ slot, index }: { slot: QuestBoardSlot; index: number }) {
  ...
  const image = QUEST_IMAGES[slot.quest_type] || combat;
  ...
  const scrapOk = (player?.hiveEngineScrap || 0) >= cost;
  ...
  return (
    <div className="relative w-64 flex-shrink-0 snap-start rounded-xl overflow-hidden border border-border/60 bg-card/40 backdrop-blur hover:border-primary/60 transition-colors block">
      ...
      <div className="p-3 space-y-3">
        <h3 className="text-sm font-bold text-foreground leading-tight min-h-[2.5rem]">
          {slot.name}
        </h3>
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground">
          ...
          <span className="text-foreground/80 font-semibold">{cost} $MGOLD</span>
        </div>
        <div className="flex flex-wrap gap-1.5">
          <ReqChip ok={levelOk}>Lv {levelReq}</ReqChip>
          <ReqChip ok={statOk}>{mapping.primary} {statReq}</ReqChip>
          <ReqChip ok={scrapOk}>{cost} $MGOLD</ReqChip>
        </div>
        <button
          onClick={doStart}
