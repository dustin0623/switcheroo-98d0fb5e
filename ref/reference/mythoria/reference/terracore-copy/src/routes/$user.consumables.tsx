...
const CATEGORIES = [
  "All", "Claim", "Attack", "Crit", "Speed", "Attack",
...
const POTIONS = [
  ...
  { name: "Attack Potion", cat: "Attack", desc: `Boost "Attack" by 50 points for 24 hours.`, price: 6.0, hue: 30 },
...
export const Route = createFileRoute("/$user/consumables")({
  head: ({ params }) => ({
    meta: [
      { title: `${params.user}'s Potions — Mythoria` },
      { name: "description", content: `${params.user}'s consumable inventory.` },
      { property: "og:title", content: `${params.user}'s Potions — Mythoria` },
      { property: "og:description", content: `${params.user}'s consumable inventory.` },
    ],
  }),
    ...
    <InventoryShell user={user} active="consumables" titleWord="Potions">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((p) => (
              <tr key={p.name} className="border-t border-border/40 hover:bg-secondary/40">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div
                      className="w-12 h-12 rounded border border-border/60 shrink-0"
                      style={{
                        background: `radial-gradient(circle at 40% 30%, oklch(0.75 0.22 ${p.hue}), oklch(0.2 0.05 260))`,
                      }}
                    />
                    <div>
