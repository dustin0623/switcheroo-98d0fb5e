...
const CATS = ["weapon", "armor", "avatar", "ship", "special"] as const;
const RARITIES = [
...
export const Route = createFileRoute("/$user/forge_logs")({
  head: ({ params }) => ({
    meta: [
      { title: `${params.user}'s Upgrade Logs — Mythoria` },
      { name: "description", content: `Forge / upgrade history for ${params.user}.` },
      { property: "og:title", content: `${params.user}'s Upgrade Logs — Mythoria` },
      { property: "og:description", content: `Forge / upgrade history for ${params.user}.` },
    ],
  }),
  component: ForgeLogsPage,
});
    ...
    return {
      item: (
        <div className="flex items-center gap-3">
          <div
            ...
            style={{
              background: `linear-gradient(135deg, oklch(0.75 0.16 ${
                (i * 53) % 360
      ...
      level: (
        <span className="font-mono">
          Lvl <span className="text-foreground">{fromLvl}</span>
          <span className="text-muted-foreground mx-2">→</span>
          <span className="text-primary">{toLvl}</span>
        </span>
      ),
      cost: (
        <span className="font-mono text-primary">{fmt(cost, 3)} ⛽</span>
      ),
    };
  });
