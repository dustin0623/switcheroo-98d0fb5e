...
import planetImg from "@/assets/planet-terracore.jpg";
import sandwormImg from "@/assets/creature-sandworm.jpg";
import { useGameStore } from "@/stores/game-store";
import { fightBoss, canFightBoss } from "@/stores/actions/boss";
...
const PLANET_KEYS = ["Terracore", "Oceana", "Celestia", "Arborealis", "Neptolith", "Solisar"] as const;
...
const planets: PlanetMeta[] = [
  {
    ...
    description:
      "A shallow warren of torchlit tunnels where goblin raiders stash the $MGOLD they steal from passing caravans. Novice adventurers cut their teeth here.",
    minLevel: 1,
    flux: 1,
    rarities: [
  ...
  },
  {
    ...
    description:
      "A silk-choked hollow deep in the Blackwood, home to venomous spiders and the ancient Broodmother that reigns over them.",
    minLevel: 10,
    flux: 2,
    rarities: [
  ...
  },
  {
    key: "Celestia",
    name: "Ancient Ruins",
    creature: "Stone Warden",
    description:
      "The half-buried remains of a forgotten empire, patrolled by animated guardians who still obey commands issued a thousand years ago.",
    minLevel: 25,
    flux: 2,
    rarities: [
  ...
  },
  {
    ...
    creature: "Prism Hydra",
    description:
      "Vast underground galleries where light refracts through veins of raw essence. The hydra that nests here has learned to weaponize its glow.",
    minLevel: 40,
    flux: 2,
    rarities: [
  ...
  },
  {
    ...
    creature: "Frost Revenant",
    description:
      "A shattered keep locked in perpetual winter. Its last king refuses to die, and rises again at every dawn to defend his hoard.",
    minLevel: 60,
    flux: 2,
    rarities: [
  ...
  },
  {
    key: "Solisar",
    name: "Dragon's Lair",
    creature: "Ancient Wyrm",
    description:
      "The final trial — a mountain hollowed out by centuries of dragonfire, guarded by the largest wyrm the realm has ever known. Only legends return from this dungeon.",
    minLevel: 80,
    flux: 2,
    rarities: [
...
export const Route = createFileRoute("/planets")({
  head: () => ({
    meta: [
      { title: "Explore Dungeons — Mythoria" },
      { name: "description", content: "Travel to distant worlds and take on new challenges." },
      { property: "og:title", content: "Explore Dungeons — Mythoria" },
      { property: "og:description", content: "Travel to distant worlds and take on new challenges." },
    ],
  }),
...
function PlanetsPage() {
  ...
  const doFight = () => {
    ...
    if (!r.ok) {
      toast.error(r.reason ?? "Fight failed");
      return;
    }
    if (r.result === "hit") {
      toast.success(`Boss defeated! Dropped ${r.rarity} crate.`);
    } else {
      toast.success(`Missed — salvaged ${r.drop}`);
    }
  };
        ...
        <section className="relative h-screen min-h-[640px] w-full overflow-hidden">
          ...
          <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-4 py-3 z-10">
            <div className="flex items-center gap-2">
              <Link to="/play" className="h-9 w-9 rounded-md border border-border/60 bg-background/60 backdrop-blur flex items-center justify-center text-foreground hover:border-primary hover:text-primary transition-colors" aria-label="Home">
              ...
              <div className="h-9 px-3 rounded-md border border-border/60 bg-background/60 backdrop-blur flex items-center gap-2 text-xs font-semibold tracking-widest">
                ...
                <span className="text-muted-foreground">·</span>
                <span className="text-primary">{player ? Math.floor(player.flux || 0) : "—"}</span>
          ...
          {/* Carousel arrows */}
          <button
            onClick={() => setIndex((i) => (i - 1 + totalSlots) % totalSlots)}
            className="absolute left-4 top-1/2 -translate-y-1/2 h-12 w-12 rounded-full bg-background/60 border border-border/60 backdrop-blur flex items-center justify-center text-foreground hover:border-primary hover:text-primary transition-colors z-10"
            aria-label="Previous dungeon"
          >
            <ChevronLeft className="h-5 w-5" />
          </button>
          <button
            onClick={() => setIndex((i) => (i + 1) % totalSlots)}
            className="absolute right-4 top-1/2 -translate-y-1/2 h-12 w-12 rounded-full bg-background/60 border border-border/60 backdrop-blur flex items-center justify-center text-foreground hover:border-primary hover:text-primary transition-colors z-10"
          ...
          <div className="absolute inset-x-0 bottom-0 px-6 md:px-10 pb-6">
            <div className="max-w-2xl">
              ...
              <h1 className="mt-1 text-5xl md:text-6xl font-bold text-foreground">
                {planet.name}
              </h1>
              <div className="mt-4 flex gap-4">
                <img
                  ...
                  width={96}
                  height={96}
                  className="h-24 w-24 rounded-md object-cover border border-border/60 flex-shrink-0"
                />
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
              <div className="flex flex-wrap gap-2">
                <Badge icon={<Star className="h-3 w-3" />} className="text-emerald-400 border-emerald-500/40">
                  Lv {planet.minLevel}+
                </Badge>
                <Badge icon={<Zap className="h-3 w-3" />} className="text-emerald-400 border-emerald-500/40">
                  {planet.flux} ESSENCE
                </Badge>
                {planet.rarities.map((r) => (
                  <Badge key={r.label} className={r.cls}>
                    {r.label}
                  </Badge>
                ))}
                <Badge icon={<Box className="h-3 w-3" />} className="text-muted-foreground border-border/60">
                  {planet.stash}
                </Badge>
                <Badge icon={<Scale className="h-3 w-3" />} className="text-muted-foreground border-border/60">
                  {planet.weight}
                </Badge>
              </div>
...
function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-md border bg-background/60 backdrop-blur px-2.5 py-1 text-xs font-semibold ${className}`}
    >
      {icon}
      {children}
    </span>
