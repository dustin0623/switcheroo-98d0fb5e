      ...
      <div className="container-tc py-10">
        <div className="flex items-center justify-between flex-wrap gap-4 mb-6">
          ...
          <div className="bracket-frame px-6 py-2 text-xs font-semibold tracking-widest text-primary">
            Rewards: {fmt(totalReward, 2)} $AETHER
          </div>
          ...
          <table className="min-w-full text-xs">
            <thead>
              <tr className="text-left text-muted-foreground uppercase tracking-widest">
                ...
                <th className="px-3 py-3 text-right">Arcane</th>
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={r.username} className="border-t border-border/50 hover:bg-accent/20">
                  ...
                  <td className="px-3 py-2 text-right">{fmt(r.stats.arcane ?? 0, 0)}</td>
                  <td className="px-3 py-2 text-right">{fmt(r.stats.damage, 0)}</td>
                  <td className="px-3 py-2 text-right">{fmt(r.stats.defense, 0)}</td>
                  <td className="px-3 py-2 text-right">{fmt(r.favor)}</td>
                  <td className="px-3 py-2 text-right">{fmt(r.staked, 3)}</td>
                  <td className="px-3 py-2 text-right">{fmt(r.aether, 3)}</td>
