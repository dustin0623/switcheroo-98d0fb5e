...
import { openCrate } from "@/features/events/crate-open/action";
import type { Crate, Rarity } from "@/features/types";
...
export default function CratesPage({ params }: { params: Promise<{ user: string }> }) {
  ...
  const isMe = user ? activeUser === user : false;
  ...
  const filtered = crates.filter((c) => cat === "All" ? true : RARITY_META[c.rarity].label === cat);
    ...
    <InventoryShell user={user} active="crates" titleWord="Chests">
      <FilterTabs
        ...
        rightSlot={
          <div className="flex items-center gap-2">
            <Link href="/shop" className="px-3 py-1.5 text-xs font-semibold border border-primary text-primary rounded-md">
              Mint Chests
            </Link>
            <Link href="/market/crates" className="px-3 py-1.5 text-xs font-semibold border border-purple-500 text-purple-400 rounded-md">
              Check Market
            </Link>
          </div>
        }
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {!inventory && (
              <tr><td colSpan={isMe ? 5 : 4} className="px-4 py-10 text-center text-muted-foreground border-t border-border/40">Loading chests…</td></tr>
            )}
            {inventory && filtered.length === 0 && (
              <tr><td colSpan={isMe ? 5 : 4} className="px-4 py-10 text-center text-muted-foreground border-t border-border/40">No chests found</td></tr>
            )}
            {filtered.map((c) => {
                      ...
                      <div
                        ...
                        style={{
                          background: `radial-gradient(circle at 30% 30%, oklch(0.75 0.18 ${
                            meta.label === "Legendary" ? 60 : meta.label === "Epic" ? 300 : meta.label === "Rare" ? 240 : meta.label === "Uncommon" ? 160 : 260
                          }), oklch(0.25 0.05 260))`,
                        }}
                      />
