...
import {
  LogsPage, RecordsBox, LogsTable, Dot, timeAgo, fmt,
} from "@/components/logs";
import { useGameStore } from "@/features/game-store/game-store";
import type { UserMarketLogEntry } from "@/features/types";

const CATEGORIES = [
  { label: "Items", href: "/market" },
  { label: "Chests", href: "/market/crates" },
  { label: "Artifacts", href: "/market/relics" },
  { label: "Consumables", href: "/market/consumables" },
];
...
export default function UserMarketLogsPage({ params }: { params: Promise<{ user: string }> }) {
  const [user, setUser] = useState<string>("");
  useEffect(() => { params.then((p) => setUser(p.user)); }, [params]);

  const cached = useGameStore((s) => (user ? s.userMarketLogs[user] : undefined));
  const fetchUserMarketLogs = useGameStore((s) => s.fetchUserMarketLogs);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!user || cached) { setLoading(false); return; }
    setLoading(true);
    fetchUserMarketLogs(user).finally(() => setLoading(false));
    ...
    return {
      item: (
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded border border-border/60" style={{ background: `linear-gradient(135deg, oklch(0.82 0.16 ${(i * 47) % 360}) 0%, oklch(0.4 0.1 260) 100%)` }} />
          <div>
      ...
      action: <span className="inline-flex items-center gap-2 text-muted-foreground"><Dot tone={actionTone(entry.action)} />{actionLabel(entry.action)}</span>,
      seller: entry.seller ? <span className={entry.seller === user ? "text-primary" : ""}>{entry.seller}</span> : <span className="text-muted-foreground">—</span>,
      buyer: entry.buyer ? <span className={entry.buyer === user ? "text-primary" : ""}>{entry.buyer}</span> : <span className="text-muted-foreground">—</span>,
      time: <span className="text-muted-foreground">{timeAgo(minutes)}</span>,
      price: entry.price != null && entry.price > 0 ? <span className="font-mono">{fmt(entry.price, 3)} HIVE</span> : <span className="text-muted-foreground">—</span>,
    };
  });
    ...
    <LogsPage
      ...
      headerRight={
        <div className="flex flex-wrap gap-2">
          {CATEGORIES.map((c) => (
            <Link key={c.label} href={c.href} className="px-3 py-1.5 text-xs font-semibold tracking-widest border border-border rounded-md text-muted-foreground hover:text-primary hover:border-primary transition-colors">
              {c.label}
            </Link>
          ))}
        </div>
