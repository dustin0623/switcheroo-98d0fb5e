...
export default function BossLogsPage({ params }: { params: Promise<{ user: string }> }) {
    ...
    <LogsPage title={`${cap(user)}'s Boss Logs`}>
      ...
      <RecordsBox title="Latest 50 Records">
        <ul className="divide-y divide-border/40">
          {records.map((r, i) => (
            <li key={i} className={`flex items-center gap-4 px-4 py-3 ${r.win ? "bg-primary/5" : ""}`}>
              <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-[10px] font-bold ${r.win ? "bg-primary/20 text-primary" : "bg-destructive/20 text-destructive"}`}>
                {r.win ? "✓" : "✕"}
              </span>
              <div className="flex-1 min-w-0">
                <div className="font-semibold">{r.boss}</div>
                <div className="text-xs text-muted-foreground">VS · {timeAgo(r.minutes)}</div>
              </div>
              <div className="hidden sm:block text-right">
                <div className="text-[10px] tracking-widest text-muted-foreground uppercase">Luck</div>
                <div className="text-sm font-mono">{luck.toFixed(2)}%</div>
              </div>
              <div className="hidden sm:block text-right">
                <div className="text-[10px] tracking-widest text-muted-foreground uppercase">Roll</div>
                <div className="text-sm font-mono">{r.roll.toFixed(2)}%</div>
              </div>
              <div className="w-32 text-right"><Pill tone={r.drop.tone}>{r.drop.label}</Pill></div>
            </li>
          ))}
        </ul>
      </RecordsBox>
