...
import {
  LogsPage, StatCard, RecordsBox, Pill,
...
export default function QuestLogsPage({ params }: { params: Promise<{ user: string }> }) {
  ...
  const records = Array.from({ length: 25 }, () => {
    ...
    return {
      ...q, completed, minutes: t,
      xp: completed ? (rand() > 0.5 ? 100 : 50) : 0,
      drops: {
        common: Math.floor(rand() * 3), uncommon: Math.floor(rand() * 3),
        rare: Math.floor(rand() * 2), epic: rand() > 0.7 ? 1 : 0, legendary: rand() > 0.95 ? 1 : 0,
      },
    };
  });
  ...
  const totalDrops = completed.reduce(
    (acc, r) => ({ common: acc.common + r.drops.common, uncommon: acc.uncommon + r.drops.uncommon, rare: acc.rare + r.drops.rare, epic: acc.epic + r.drops.epic, legendary: acc.legendary + r.drops.legendary }),
    ...
    <LogsPage title={`${cap(user)}'s Quest Log`}>
      ...
      <RecordsBox title="Latest 25 Records">
        <ul className="divide-y divide-border/40">
          {records.map((r, i) => (
            <li key={i} className={`flex items-center gap-4 px-4 py-3 ${r.completed ? "bg-primary/5" : ""}`}>
              <div className="text-2xl">{r.icon}</div>
              <div className="w-10 shrink-0"><Pill tone="warning">{r.tier}</Pill></div>
              <div className="flex-1 min-w-0">
                <div className="font-semibold">{r.name}</div>
                <div className="text-xs text-muted-foreground mt-0.5">
                  {r.completed ? (<><span className="text-primary">✓ Completed</span> {timeAgo(r.minutes)}</>) : (<><span className="text-yellow-400">▶ Started</span> {timeAgo(r.minutes)}</>)}
                </div>
              </div>
              {r.completed && (
                <div className="flex items-center gap-2 flex-wrap justify-end">
                  {r.drops.common > 0 && <Pill tone="muted">{r.drops.common}× C</Pill>}
                  {r.drops.uncommon > 0 && <Pill tone="success">{r.drops.uncommon}× U</Pill>}
                  {r.drops.rare > 0 && <Pill tone="info">{r.drops.rare}× R</Pill>}
                  {r.drops.epic > 0 && <Pill tone="warning">{r.drops.epic}× E</Pill>}
                  {r.drops.legendary > 0 && <Pill tone="danger">{r.drops.legendary}× L</Pill>}
