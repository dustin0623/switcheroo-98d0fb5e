...
import { equipItem, forgeItem, salvageItem, unequipSlot } from "@/features/events/item-manage/action";
import type { Item, Rarity } from "@/features/types";
import { toast } from "sonner";
import type { Metadata } from "next";

const CATEGORIES = ["All", "Equipped", "mount", "weapon", "armor", "avatar", "artifact", "For Sale"] as const;
...
export default function ItemsPage({ params }: { params: Promise<{ user: string }> }) {
  ...
  const player = useGameStore((s) => (user ? s.players[user] ?? null : null));
          ...
          <tbody>
            ...
            {filtered.map((it, i) => {
              ...
              return (
                <tr
                  key={it._id}
                  className={`border-t border-border/40 hover:bg-secondary/40 ${it.equiped ? "bg-primary/5" : ""}`}
                >
                  ...
                  <td className="px-4 py-3"><Pill tone="muted">{it.edition}</Pill></td>
                  ...
                  <td className="px-4 py-3 text-right text-muted-foreground">
                    <span className="text-foreground">{it.print}</span> of<br />{it.max_supply.toLocaleString()}
                  </td>
                  <td className="px-4 py-3 text-right font-mono text-xs whitespace-pre-line">
                    {active.length
                      ? active.map(([k, v]) => `${k}: +${v.toFixed(k === "damage" || k === "defense" ? 2 : 3)}`).join("\n")
                      : "—"}
                  </td>
                          ...
                          <>
                            ...
                            <button
                              onClick={() => {
                                ...
                                if (r.ok) toast.success(`Salvaged ${it.name} for ${r.essence?.toFixed(3)} essence`);
