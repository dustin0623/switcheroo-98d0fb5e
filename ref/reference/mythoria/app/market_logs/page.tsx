...
function generateRows(count: number, seedStr: string) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    rows.push({
      ...
      price: <span className="font-mono">{fmt(price, 3)} HIVE</span>,
    });
...
export default function MarketLogsPage() {
  ...
  return (
    <LogsPage
      ...
      headerRight={
        <div className="flex gap-2">
          <button
            type="button"
            className="px-4 py-2 text-xs font-semibold tracking-widest border border-border rounded-md text-muted-foreground hover:text-foreground"
          >
            Previous
          </button>
          <button
    ...
    >
      <RecordsBox title="Recent Transactions">
        <LogsTable
          columns={[
            { key: "item", label: "Item" },
