 ...
 *
 * Reads from environment (via src/lib/config/config.ts):
 *   MONGODB_URI        — MongoDB connection string (required)
 *   TREASURY_ADDRESS   — treasury wallet that receives deposits
 *   TREASURY_KEY       — treasury secret key (base58) used for payouts
 *   CONTRACT_ADDRESS   — SPL mint of the game token
 *   SOLANA_RPC_URL     — JSON-RPC endpoint
 *   WORKER_POLL_MS     — drain interval (default 5000)
 *   WORKER_MAX_RETRIES — transient-failure ceiling (default 8)
 *
 * OPERATIONAL CONSTRAINT: run exactly ONE instance so settlement stays
 * sequential and oldest-first.
 * SERVER-ONLY — never imported by the app.
 */
