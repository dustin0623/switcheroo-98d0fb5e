...
import { listItem } from "@/lib/modules/market-listings/repository.server";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { itemNumber, price } = listInput.parse(body);
    const result = await listItem({ seller: auth.wallet, itemNumber, price });
    if (result.ok) {
      await createLog({
        type: "market",
        wallet: auth.wallet,
