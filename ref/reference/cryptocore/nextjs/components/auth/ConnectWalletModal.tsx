...
export function ConnectWalletModal({
  ...
  const { online, loading: statusLoading } = useApiStatus();
  const phantomInstalled = typeof window !== "undefined" && isPhantomInstalled();
  const serverOffline = online === false;
  ...
  function handlePlayDemo() {
    const address = playDemo();
    toast.success("Demo mode — progress is saved on this device only");
    return address;
  }
        ...
        {step === "wallet" ? (
          <>
            ...
            {serverOffline && (
              <div className="flex items-start gap-2 rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-[12px] text-destructive">
                ...
                <span>
                  Server offline — wallet sign-in is unavailable right now. You can still explore the
                  game in demo mode.
                </span>
              </div>
            )}
            ...
            <div className="space-y-2">
              {WALLETS.map((wallet) => {
                ...
                const disabled =
                  ...
                  serverOffline ||
                  "disabled" in wallet ||
                  (isPhantom && !notInstalled && online !== true);
                const hint = "disabled" in wallet
                  ? wallet.hint
                  : serverOffline
                    ? "Server offline"
                    : notInstalled
                ...
                return (
                  <Button
                    ...
                    disabled={disabled && !(notInstalled && !serverOffline)}
          ...
          <>
            ...
            >
              ...
              <Button type="submit" className="h-11 w-full gap-2" disabled={!usernameValid || saving}>
