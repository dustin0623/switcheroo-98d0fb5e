 ...
 */
export const vaultCapacity = (vaultStaked: number, totalHashRate = 0): number => {
  const rigOutput = Math.pow(effectiveHashRate(totalHashRate) + 1, 2);
  return Math.round(
    Math.max(VAULT_BASE_CAPACITY, rigOutput) +
      Math.max(0, vaultStaked) * (1 + VAULT_CAPACITY_PER_LEVEL),
  );
};
 ...
 */
export const decayMultiplier = (lastSinkAt: number | null | undefined, now = Date.now()): number => {
...
/**
 * HASH mined per second. Scales with (effective hash rate + 1)², calibrated
 * so the next upgrade is affordable roughly every UPGRADE_TARGET_SECONDS.
 */
export const miningPerSecond = (totalHashRate: number, decay = 1): number => {
  ...
  return (nextUpgradeCost / UPGRADE_TARGET_SECONDS) * MINING_MULTIPLIER * Math.max(0, decay);
};
