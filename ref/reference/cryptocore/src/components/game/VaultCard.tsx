  ...
  return (
    <div className="card-soft relative overflow-hidden p-6">
      ...
      <div className="relative space-y-5">
        ...
        <motion.div animate={claiming ? { scale: [1, 1.06, 1] } : {}} transition={{ duration: 0.4 }}>
          ...
          <p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground tabular-nums">
            ...
            <motion.span
              key={formatDuration(secondsToFull)}
              initial={{ opacity: 0.4 }}
              animate={{ opacity: 1 }}
            >
              {full ? "mining halted" : `Full in ${formatDuration(secondsToFull)}`}
            </motion.span>
          </p>
        </motion.div>
        ...
        <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
          <span className="tabular-nums">{formatHash(perSecond)} HASH / sec</span>
