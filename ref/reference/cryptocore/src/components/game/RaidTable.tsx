...
export function RaidTable() {
  ...
  useEffect(() => {
    if (scaledFor !== total.hackPower) refreshRivals(total.hackPower);
  }, [scaledFor, total.hackPower, refreshRivals]);
  ...
  return (
    <section className="space-y-3">
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 sm:flex sm:flex-wrap sm:justify-between">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <span className="hidden rounded-lg border border-border bg-secondary/50 px-3 py-1.5 text-xs text-muted-foreground sm:inline-flex sm:items-center sm:gap-2">
            <span className="font-semibold text-foreground">
              Charges: {raidChargeInfo.current}
            </span>
        ...
        <Button variant="secondary" size="sm" onClick={() => refreshRivals(total.hackPower)}>
      ...
      <div className="grid gap-2 sm:hidden">
        {rivals.map((rival) => {
          const chance = raidSuccessChance(total.hackPower, rival.security, rival.firewall);
          const beatable = canRaid(total.hackPower, rival.security);
          return (
                ...
                <div className="min-w-0">
                  ...
                  <p
                    className={cn(
                      ...
                      chance >= 65 ? "text-success" : chance >= 40 ? "text-primary" : "text-danger",
                    )}
                  >
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">Firewall</TableHead>
