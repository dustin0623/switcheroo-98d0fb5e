...
export function simulateRaid(
  ...
  if (defender.protectionUntil > Date.now()) {
    return { success: false, reason: "blocked", chance: 0, stealPercent: 0, stolen: 0, xp: 0, serverSeed: "" };
  }
  ...
  if (!success) {
    return { success: false, reason: "failed", chance, stealPercent: 0, stolen: 0, xp: 0, serverSeed };
  }
