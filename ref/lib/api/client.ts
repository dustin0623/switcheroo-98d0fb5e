 ...
 */
export async function getMarketListings(): Promise<{ ok: boolean; listings?: MarketListingDto[]; error?: string }> {
  try {
 ...
 */
export async function deposit(txId: string, amount: number): Promise<{
  ok: boolean;
  queued?: boolean;
  jobId?: string;
  txId?: string;
  ...
  try {
    return await request<{ ok: boolean; queued: boolean; jobId: string; txId: string; amount: number }>(
      "/api/game/deposit",
      {
        method: "POST",
...
export async function getWalletBalance(): Promise<{ ok: boolean; balance?: number; error?: string }> {
  try {
    return await request<{ ok: boolean; balance: number }>("/api/wallet/balance", { method: "GET" });
  } catch (err) {
    return { ok: false, error: err instanceof ApiError ? err.message : "Network error" };
  }
}
 ...
 */
export async function getTreasuryBalance(): Promise<{ ok: boolean; balance?: number; error?: string }> {
  try {
