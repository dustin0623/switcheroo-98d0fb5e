...
export async function POST(request: Request) {
  ...
  if (!treasuryAddress) {
    return jsonResponse({ ok: false, error: "TREASURY_ADDRESS is not set" }, request, { status: 500 });
  }
  if (!mintAddress) {
    return jsonResponse({ ok: false, error: "CONTRACT_ADDRESS (mint) is not set" }, request, { status: 500 });
  }
  ...
  if (!parsed.success) {
    return jsonResponse({ ok: false, error: "amount must be a positive integer" }, request, { status: 400 });
  }
  ...
  try {
    ...
    return jsonResponse({ ok: false, error: "Invalid wallet address on session" }, request, { status: 400 });
  }
  ...
  try {
    ...
    return jsonResponse({ ok: false, error: "Failed to fetch mint info" }, request, { status: 502 });
  }
  ...
  try {
    ...
    return jsonResponse({ ok: false, error: "Failed to check your token account" }, request, { status: 502 });
  }
  ...
  try {
    ...
    return jsonResponse({ ok: false, error: "Failed to fetch a recent blockhash" }, request, { status: 502 });
  }

  const tx = new Transaction({ feePayer: payerPk, recentBlockhash: blockhash }).add(...instructions);
