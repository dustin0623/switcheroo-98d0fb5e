...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!isChainPaymentConfigured()) {
      return jsonResponse(
        { ok: false, error: "Withdrawals aren't available yet — on-chain payouts aren't configured." },
        request,
        { status: 503 },
      );
    }
    ...
    if (!debited.ok) {
      return jsonResponse(
        { ok: false, error: "Insufficient HASH balance" },
        request,
        { status: 400 },
      );
    }
    ...
    return jsonResponse(
      { ok: true, queued: true, jobId, signature, amount },
      request,
      { status: 202 },
    );
  } catch (err) {
