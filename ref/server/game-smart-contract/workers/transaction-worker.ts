...
export class TransactionWorker {
    ...
    try {
      ...
      if (!verification.ok) {
        if (verification.code === "NOT_CONFIRMED") {
          ...
          if (dead) {
            await this.finalizeDead(job, "deposit", verification.error ?? "not confirmed (max retries)");
            return;
          }
    ...
    try {
      ...
      if (!verification.ok) {
        if (verification.code === "NOT_CONFIRMED") {
          const dead = await failJob(
            id, verification.error ?? "payment not confirmed", this.maxRetries,
          );
          if (dead) {
            await this.finalizeDead(
              job, "market", verification.error ?? "payment not confirmed (max retries)",
            );
            return;
          }
      ...
      if (!transferOk) {
        await this.refundMarketPurchase(
          job, price, `NOT_FOUND: ${transferError || "transfer failed"}`,
        );
        await this.finalizeDead(
          job, "market", `NOT_FOUND: ${transferError || "transfer failed"}`,
        );
        return;
      }
      ...
      await createLog({
        type: "market", wallet: buyer, target: seller,
        amount: -price, txHash: paymentTxId,
        data: { action: "bought", ...detail },
      });
      await createLog({
        type: "market", wallet: seller, target: buyer,
        amount: sellerProceeds, txHash: payout.signature,
      ...
      logger.info("settled market purchase", {
        buyer, seller, itemNumber, price, payout: payout.signature,
      });
    } catch (err) {
  ...
  private async refundMarketPurchase(
    ...
    try {
      ...
      logger.warn("refunded market purchase", {
        buyer, amount, itemNumber: job.itemNumber, reason,
      });
    } catch (err) {
      await createErrorLog({
        type: "market",
  ...
  private async finalizeDead(
    ...
    await completeJob(id);
    await createErrorLog({ type: label, wallet: job.walletAddress, error: reason, data: { dead: true, jobId: id } });
    logger.warn(`dead-lettered ${label} — removed from pending queue`, { wallet: job.walletAddress, reason });
  }
  ...
  private async retryOrDie(
    ...
    await createErrorLog({ type: label, wallet: job.walletAddress, error: message(err), data: { dead: false, jobId: id } });
