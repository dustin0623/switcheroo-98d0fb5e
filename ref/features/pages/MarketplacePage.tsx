...
import { RARITY_KEYS, RARITY_META, SLOT_KEYS, SLOT_META, STAT_KEYS } from "@/features/constants/game";
...
export function MarketplacePage() {
  ...
  const load = useCallback(async () => {
    ...
    if (result.ok && result.listings) {
      ...
      setAssetListings(
        result.listings.map(toServerAssetListing).filter((l): l is ServerAssetListing => l !== null),
      );
    }
    setLoading(false);
  }, []);
    ...
    <div className="mt-auto space-y-2">
      ...
      {isOwn ? (
        <div className="space-y-1">
          ...
          <Button size="sm" variant="outline" className="w-full" disabled={isCancelling} onClick={onCancel}>
            {isCancelling ? "Cancelling…" : "Cancel listing"}
          </Button>
        </div>
      ) : (
        <Button size="sm" className="w-full" disabled={(walletConnected && !canAfford) || isBuying} onClick={onBuy}>
          {isBuying ? (
...
function AssetListingCard({
  ...
  const KindIcon = isAvatar ? User : listing.cosmeticKind === "banner" ? RectangleHorizontal : ImageIcon;
    ...
    <article className="card-soft flex flex-col gap-3 p-4 ring-1 ring-inset ring-border/50">
      ...
      <div className={cn(isAvatar && "text-center")}>
        ...
        <p className="mt-0.5 flex items-center gap-1 text-xs text-muted-foreground">
          ...
          <span className={cn(isAvatar && "sr-only")}>{COSMETIC_KIND_LABEL[listing.cosmeticKind]}</span>
        </p>
      </div>
      {children}
    </article>
