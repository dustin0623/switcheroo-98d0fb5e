...
export const resolveRaid = ({ hackPower, exploit, enemy, vaultSpace }: RaidInput): RaidOutcome => {
  ...
  if (!canRaid(hackPower, enemy.security)) {
    return { success: false, reason: "outmatched", chance, stealPercent: 0, stolen: 0, takenFromVault: 0 };
  }
  ...
  if (firewallRoll <= clamp(enemy.firewall, 0, 100)) {
    return { success: false, reason: "blocked", chance, stealPercent: 0, stolen: 0, takenFromVault: 0 };
  }
  ...
  const stolen =
    vaultSpace !== undefined ? Math.min(takenFromVault, Math.max(0, Math.floor(vaultSpace))) : takenFromVault;
