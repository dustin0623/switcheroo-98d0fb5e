...
const initialState = (): PlayerStats => ({
  ...
  milestones: { totalClaimed: 0, totalMined: 0, raids: 0, raidWins: 0, totalStolen: 0, bestHashRate: 1 },
  notoriety: 0,
  totalBurned: 0,
  referralCount: 0,
  referralEarned: 0,
  ...
  persist(
    (set, get) => ({
      ...
      tick: (totalHashRate, seconds) => {
        ...
        set({
          vault: result.vault,
          milestones: { ...state.milestones, totalMined: state.milestones.totalMined + result.mined },
          lastTickAt: Date.now(),
        });
      ...
      claim: async () => {
        ...
        set({
          ...
          vault: 0,
          milestones: { ...state.milestones, totalClaimed: state.milestones.totalClaimed + claimed },
