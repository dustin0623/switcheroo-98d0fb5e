...
export function EditCosmeticsModal({ open, onOpenChange }: EditCosmeticsModalProps) {
  const { data: assets = [], isLoading, mutate } = useSWR<OwnedAsset[]>(
    open ? "owned-assets" : null,
    fetchAssets,
  ...
  async function handleSave() {
    ...
    if (selectedAvatar     !== null && selectedAvatar     !== equippedAvatar?.assetNumber)     ops.push(equipAsset(selectedAvatar));
    if (selectedBanner     !== null && selectedBanner     !== equippedBanner?.assetNumber)     ops.push(equipAsset(selectedBanner));
    if (selectedBackground !== null && selectedBackground !== equippedBackground?.assetNumber) ops.push(equipAsset(selectedBackground));
  ...
  const canSellSelected = Boolean(selectedAsset) && !selectedAsset!.soulbound && !selectedAsset!.equipped;
  ...
  async function handleSell() {
    ...
    notify(`${tpl?.name ?? "Cosmetic"} listed for ${formatHash(listPrice)} HASH on the marketplace`, "success");

    // Clear any selection pointing at the now-listed asset, then refresh.
    if (sellTarget.kind === "avatar"     && selectedAvatar     === sellTarget.assetNumber) setSelectedAvatar(equippedAvatar?.assetNumber ?? null);
    if (sellTarget.kind === "banner"     && selectedBanner     === sellTarget.assetNumber) setSelectedBanner(equippedBanner?.assetNumber ?? null);
    if (sellTarget.kind === "background" && selectedBackground === sellTarget.assetNumber) setSelectedBackground(equippedBackground?.assetNumber ?? null);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[85vh] w-full max-w-2xl flex-col gap-0 overflow-hidden p-0">
        ...
        <Tabs value={tab} onValueChange={(value) => setTab(value as typeof tab)} className="flex min-h-0 flex-1 flex-col">
          <TabsList className="mx-6 mt-4 shrink-0 justify-start gap-1 rounded-lg bg-secondary/50 p-1">
            <TabsTrigger value="avatar"     className="text-xs font-medium">Avatar ({avatarAssets.length})</TabsTrigger>
            <TabsTrigger value="banner"     className="text-xs font-medium">Banner ({bannerAssets.length})</TabsTrigger>
            <TabsTrigger value="background" className="text-xs font-medium">Background ({backgroundAssets.length})</TabsTrigger>
          </TabsList>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        selected ? "ring-2 ring-primary bg-primary/10" : "ring-1 ring-border/50 hover:ring-border",
                      )}
                      title={tpl.name}
                    >
                      ...
                      <span className="line-clamp-1 w-full text-center text-[9px] text-muted-foreground leading-tight">{tpl.name}</span>
          ...
          <TabsContent value="banner" className="mt-0 min-h-0 flex-1 overflow-y-auto px-6 py-4">
            {isLoading ? <LoadingGrid cols={3} aspect="3/1" /> : (
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        selected ? "ring-2 ring-primary" : "ring-1 ring-border/50 hover:ring-border",
                      )}
                      title={tpl.name}
                    >
      ...
      {/* Sell confirmation */}
      <AlertDialog open={Boolean(sellTarget)} onOpenChange={(isOpen) => (isOpen ? null : setSellTarget(null))}>
        <AlertDialogContent>
          {sellTarget && sellTpl ? (
            <>
              <AlertDialogHeader>
              ...
              <div className="flex items-center gap-3 border-b border-border pb-4">
                <div
                  className={cn(
                    ...
                    sellTarget.kind === "avatar" ? "size-14 rounded-full" : "aspect-[3/1] w-24 rounded-lg",
                  )}
                >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {sellTarget.kind.charAt(0).toUpperCase() + sellTarget.kind.slice(1)} · #{sellTarget.mintNumber}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <p className="text-sm">
                  List this cosmetic on the in-game marketplace. When it sells, you receive the price minus a{" "}
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-right text-xs text-muted-foreground">
                    ...
                    <span className="font-semibold text-success">{formatHash(net)} HASH</span> after fees
                  </p>
                </div>
              </div>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction onClick={handleSell} disabled={!priceValid || selling}>
                  {selling ? <><Loader2 className="mr-2 size-4 animate-spin" />Listing…</> : "List for sale"}
                </AlertDialogAction>
              </AlertDialogFooter>
            </>
          ) : null}
...
function MintBadge({ asset, className }: { asset: OwnedAsset; className?: string }) {
  if (asset.soulbound) {
    return (
      <span className={cn("flex items-center gap-0.5 rounded-full bg-background/80 px-1.5 py-0.5 text-[8px] font-medium text-muted-foreground", className)}>
        <Lock className="size-2.5" /> Default
      </span>
    );
  }
  return (
    <span className={cn("rounded-full bg-background/80 px-1.5 py-0.5 text-[8px] font-medium text-muted-foreground", className)}>
      #{asset.mintNumber}
    </span>
  );
}
...
function LoadingGrid({ cols = 6, aspect }: { cols?: number; aspect?: string }) {
  return (
    <div className={cn("grid gap-3", cols === 6 ? "grid-cols-4 sm:grid-cols-6" : "grid-cols-2 sm:grid-cols-3")}>
      {Array.from({ length: cols * 2 }).map((_, i) => (
        <div
          key={i}
          className={cn("animate-pulse rounded-xl bg-secondary/50", aspect ? `aspect-[${aspect}]` : "aspect-square")}
        />
      ))}
    </div>
  );
