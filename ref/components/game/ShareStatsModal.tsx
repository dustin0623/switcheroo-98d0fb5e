...
export function ShareStatsModal({ open, onClose, cardProps }: ShareStatsModalProps) {
  ...
  useEffect(() => {
    ...
    if (cardProps.avatarImage) {
      toDataUrl(cardProps.avatarImage).then(setResolvedAvatar).catch(() => setResolvedAvatar(cardProps.avatarImage));
    }
    if (cardProps.bannerImage) {
      toDataUrl(cardProps.bannerImage).then(setResolvedBanner).catch(() => setResolvedBanner(cardProps.bannerImage));
    }
